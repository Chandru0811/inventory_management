...
const VendorEdit = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      formData.append("enablePortal", values.enablePortal);
      ...
      formData.append(
        ...
        JSON.stringify(
          rows
            .map((row) => row.contacts?.[0]) // Extract the contact object directly
            .filter(
              (item) =>
                item.salutation &&
      ...
      console.log("Contacts Data:", values.contacts);
      formData.append(
        "bankDetails",
        JSON.stringify(
          values.bankDetails
      ...
      try {
        const response = await api.put(`/updateVendorWithBank/${id}`, formData, {});
    ...
    const getData = async () => {
      try {
        ...
        formik.setValues({
          ...
          contacts: response.data.vendorContactPersonsModels.map((contact) => ({
